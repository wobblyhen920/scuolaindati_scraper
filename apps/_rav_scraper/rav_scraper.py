...
"""
Scraper SNV / RAV asincrono.
Output long atomico per INDEX + merge finale, fallback scuola->istituto,
parsing tabelle multi-riga, estrazione spunte/checkbox, normalizzazione label.
"""
...
def _has_img(cell) -> bool:
    """True se la cella contiene una spunta (img, svg, css background, checkbox checked, ecc.)."""
    if cell is None:
        return False
...
def var_bold_p_headings_before(table, max_steps: int = 200) -> List[str]:
    """Raccoglie <p> precedenti con 'classi' e bold."""
    headings: List[str] = []
...
def var_extract_tables(soup: BeautifulSoup) -> List[Dict[str, Any]]:
    """Estrae intestazioni e prima riga 'Situazione' per ogni tabella .tableDsc."""
...
def parse_ep_2_4_a_4(soup: BeautifulSoup) -> List[dict]:
    """INVALSI (2.4.a.4): per ogni tabella .tableDsc, prima riga con 'classi' + colonne punteggio/escs."""
    records: List[dict] = []
...
def _parse_snv_table_with_spunte(table, expected_cols: List[str]) -> List[dict]:
    """Estrae righe 'Situazione...' e colonne attese. Celle con immagine -> True."""
...
def parse_ep_2_2_a_1(soup: BeautifulSoup) -> List[dict]:
    """Endpoint 2.2.a.1 — punteggio prove e differenze ESCS. Logica 'first classi' per tabelle .tableDsc."""
...
def records_to_long_rows(
    ...
    """Converte records in formato long."""
    out=[]
...
async def scarica_long_safe_async(
    ...
    """Scarica e parsa i 3 descrittori: 1.1.b.1, 2.2.b.2, 2.2.a.1."""
    rows: List[dict] = []
    if not endpoints:
        return []
