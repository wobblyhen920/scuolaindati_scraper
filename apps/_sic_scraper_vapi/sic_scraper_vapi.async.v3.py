...
"""
sic_scraper_async.py

Batch scraper ScuolaInChiaro / UNICA (API reverse engineered).

Principi
- Salva SEMPRE payload raw su disco: outdir/raw/<CODICE_SCUOLA>/<endpoint>.json|.txt
- Checkpoint PER SCUOLA: outdir/blocks_partial/<CODICE_SCUOLA>.csv
  - Resume: se il checkpoint esiste, la scuola viene saltata
  - --fresh: elimina checkpoint e forza nuovo run (raw sovrascritti)
- Post-processing (long) in streaming: niente RAM massiva

Scelte tecniche consigliate
- Worker pool async con coda: backpressure reale
- Concurrency distinta:
  - --conn-limit: connessioni TCP totali
  - --inflight: richieste HTTP contemporanee (semaforo)
- Retry “sensato”:
  - retry su 429/5xx/timeout/connection reset
  - NO retry su 400/401/403/404 (salvo scelta esplicita)
  - rispetto Retry-After se presente
- Timeout separati: connect / sock_read / total

Miglioria (2026-01)
- Progress aggregato periodico (requests, scuole, errori, 429, rate, coda)
- Checkpoint "streaming" appena una scuola completa tutti gli endpoint (meno RAM, resume migliore)
"""
...
async def fetch_with_retry(
    ...
    *,
    ...
    """
    Return: (status, final_url, content_type, text, err)
    ...
    *,
    ...
    while True:
        ...
        async with lock:
            ...
            # se scuola completata -> checkpoint immediato e drop in RAM
            if done_now:
...
async def run(args: argparse.Namespace) -> None:
    ...
    # Safety net: se per qualunque motivo restano risultati non checkpointati
    # (es. expected_per_school cambia, o job incompleti), li checkpointiamo qui.
