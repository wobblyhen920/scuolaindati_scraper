...
# -*- coding: utf-8 -*-
"""
release_roller.py — costruzione release (long -> wide).

Canonicalizza CODICE_SCUOLA, dedup 1-riga-per-scuola,
riordino colonne (anagrafica first, poi variabili per prefisso numerico).
Merge opzionali: SNAI, ISTAT comuni, sic-studenti, SNV/RAV.
"""
...
def load_sic_studenti_csv(fp: Path) -> pd.DataFrame:
    """Carica sic-studenti in modo robusto (gestisce virgole non quotate nei campi)."""
    try:
...
def _norm_place_key(x: str) -> str:
    """Normalizza nomi comuni per match robusto (maiuscole, no accenti, no punteggiatura)."""
    s = "" if x is None else str(x)
    s = _strip_accents(s)
    s = s.strip().upper()
    s = s.replace("’", "'")
    # Citta' -> CITTA (apostrofo dopo vocale)
...
def _normalize_cap(x: object) -> str:
    """Normalizza CAP a 5 cifre con padding a sinistra. Non numerico -> ''."""
    if x is None:
        return ""
...
def to_float_or_nan(s: pd.Series) -> pd.Series:
    """Parsing numerico: gestisce formato europeo (1.234,56) e normale."""
...
def _star_computed_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Prefissa '*' alle colonne calcolate (non quelle scraped/pivotate). Idempotente."""
    ren: dict[str, str] = {}
    for c in df.columns:
        if c.startswith("*"):
            continue
...
def _year_from_category(cat: str) -> Optional[int]:
    """Estrae indice temporale da categorie SIC (y1..y6, anno1..6, 2020/21, ecc.)."""
    if cat is None:
        return None
    c = str(cat).strip()
    if not c:
...
def _linear_slope(x: np.ndarray, y: np.ndarray) -> float:
    """Pendenza lineare (y ~ a + b*x), NaN se <2 punti."""
    import warnings
...
def _linear_slope_indexed(yvals):
    """Slope su indici 1..n, ignora NaN, ritorna NaN se <2 punti."""
...
def _add_forced_slopes(df: pd.DataFrame) -> pd.DataFrame:
    """Ricalcola slope per abbandoni/ripetenti/trasferimenti direttamente da y1..y5."""
...
def load_istat_comuni_xlsx(fp: Path) -> pd.DataFrame:
    """Carica elenco comuni ISTAT, prepara chiave di join su comune + provincia."""
    df = pd.read_excel(fp)
...
def merge_regione_macroarea(wide: pd.DataFrame, istat_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Aggiunge 'regione' e 'macroarea' al wide. Match su (comune, provincia), fallback solo comune."""
    w = wide.copy()
...
def build_snv_wide_from_clean_csv(
    ...
    """Build wide table per scuola dal CSV long SNV già pulito."""
...
def _update_latest_symlink(releases_root: Path, latest_target: Path) -> None:
    """Crea/aggiorna il symlink 'latest' nella cartella release."""
...
def _merge_snai(df_wide: pd.DataFrame, snai_path: Path, comune_col_wide: str = "comune") -> pd.DataFrame:
    """Merge classificazione SNAI su wide usando nome comune normalizzato."""
